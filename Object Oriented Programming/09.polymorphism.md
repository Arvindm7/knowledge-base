...
- What Polymorphism is and why it is important in OOP
- The two main types of polymorphism: Compile-Time and Run-Time
- How Method Overloading achieves Compile-Time Polymorphism
- How Method Overriding achieves Run-Time Polymorphism
...
> [!NOTE]
> The return type **cannot** be a differentiator for Method Overloading. Only the parameter list (number, type, or order of parameters) differentiates overloaded methods.

---

## 2. Run-Time Polymorphism (Dynamic Polymorphism)

In run-time polymorphism, the method is **resolved during runtime**. It is achieved through **method overriding**. When we say the method is "resolved" at run-time, it refers to the decision about which method to call being made at the time the program is actually running. This occurs due to the **dynamic method dispatch** mechanism, where the JVM decides which method of a subclass to call based on the **actual object type** (not the reference type) at runtime.
...
Here, the method to be executed is decided at runtime based on the **actual object type**.
...
| :--- | :--- | :--- | :--- |
| **Compile-Time Polymorphism** | Static Polymorphism | Method Overloading | Compile-time |
| **Run-Time Polymorphism** | Dynamic Polymorphism | Method Overriding | Runtime |
